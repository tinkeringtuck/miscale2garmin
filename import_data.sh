...
while [[ $loop_count -eq 0 ]] || [[ $i -lt $loop_count ]] ; do
	...
	if [[ $switch_miscale == "on" && $switch_mqtt == "off" ]] || [[ $switch_omron == "on" ]] ; then
		...
		source <(grep miscale_ble_hci $path/user/export2garmin.cfg)
	...
	if [[ $switch_miscale == "on" ]] ; then
		...
		if [[ $switch_mqtt == "on" ]] ; then
			...
			if [[ $ble_status == "ok" ]] ; then
				...
				miscale_read=$(echo $ble_check | awk '{sub(/.*BLE scan/, ""); print substr($1,1)}')
				miscale_unixtime=$(echo $miscale_read | awk -F ";" '{print $1}')
			fi
		fi
	...
	if [[ $switch_omron == "on" ]] ; then
		...
		if [[ $ble_status == "ok" ]] ; then
			echo "$(timenow) OMRON * Importing data from a BLE scanner"
